...
def create_author(
    args: AuthorCreateRequest, current_user: CurrentUser, service: AuthorServiceDepends, translator: Translator
):
    if service.exists(args.name):
        raise HTTPException(
            status.HTTP_409_CONFLICT, translator.gettext("An author with the name '%s' already exists." % (args.name,))
        )
...
def get_author(name: str, service: AuthorServiceDepends, translator: Translator):
    author = service.get_author_by_name(name)

    if not author:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, translator.gettext("No author found with the name '%s'." % (name,))
        )
...
def get_authors(search_params: SearchParamsDepends, service: AuthorServiceDepends, translator: Translator):
    authors = service.get_authors(search_params)

    if not authors:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, translator.gettext("No authors found matching the provided search parameters.")
        )
